...
|| ($action == 'updateedit')) {
  ...
  dolibarr_set_const($db, "MAIN_INFO_SOCIETE_FSHAT", GETPOST("fshat", 'alphanohtml'), 'chaine', 0, '', $conf->entity);
...
// Fshati
print '<tr class="oddeven"><td><label for="fshat">'.$langs->trans("Fshat").'</label></td><td>';
print img_picto('', 'object_phoning_fax', '', false, 0, 0, '', 'pictofixedwidth');
print '<input class="maxwidth150" name="fshat" id="fshat" value="'.dol_escape_htmltag((GETPOSTISSET('fshat') ? GETPOST('fshat', 'alphanohtml') : (!empty($conf->global->MAIN_INFO_SOCIETE_FSHAT) ? $conf->global->MAIN_INFO_SOCIETE_FSHAT : ''))).'"></td></tr>';
print '</td></tr>'."\n";
