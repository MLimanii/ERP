...
function getCyrillic($string) {

  $cyrillic = ['Љ', 'Њ', 'Џ', 'џ', 'ш', 'ќ', 'ч', 'ћ', 'ж', 'љ', 'њ', 'Ш', 'Ќ', 'Ч', 'Ћ', 'Ж','Ц','ц', 'а','б','в','г','д','е','ё','ж','з','и','й','к','л','м','н','о','п', 'р','с','т','у','ф','х','ц','ч','ш','щ','ъ','ы','ь','э','ю','я', 'А','Б','В','Г','Д','Е','Ё','Ж','З','И','Й','К','Л','М','Н','О','П', 'Р','С','Т','У','Ф','Х','Ц','Ч','Ш','Щ','Ъ','Ы','Ь','Э','Ю','Я'
  ];

  $lattin = ['Lj', 'Nj', 'Xh', 'xh', 'sh', 'q', 'č', 'ć', 'ž', 'lj', 'nj', 'Sh', 'Q', 'Č', 'Ć', 'Ž','C','c', 'a','b','v','g','d','e','io','zh','z','i','y','k','l','m','n','o','p', 'r','s','t','u','f','h','ts','ch','sh','sht','a','i','y','e','yu','ya', 'A','B','V','G','D','E','Io','Zh','Z','I','Y','K','L','M','N','O','P', 'R','S','T','U','F','H','Ts','Ch','Sh','Sht','A','I','Y','e','Yu','Ya'
  ];

  $pattern = "/^[\p{Cyrillic}\p{Common}]+$/u";
  $result = (bool) preg_match($pattern, $string);

  if ($result) {
    return str_replace($lattin, $cyrillic, $string);
  }else {
    return str_replace($lattin, $cyrillic, $string);
  }
...
function getLattin($string) {
  ...
  if ($result) {
    return str_replace($cyrillic, $lattin, $string);
  }else {
    return str_replace($cyrillic, $lattin, $string);
  }
...
function getRegions() {
  ...
  $regions = [
    1 => $langs->trans("Saraj"),
    2 => $langs->trans("Lubin"),
    3 => $langs->trans("Krushopek"),
    4 => $langs->trans("Gërçec"),
    5 => $langs->trans("Gullmovë"),
    6 => $langs->trans("Lloka"),
    7 => $langs->trans("Shishovë"),
    9 => $langs->trans("Kondovë"),
    16 => $langs->trans("Rashçe"),
    17 => $langs->trans("Arnaqi"),
    18 => $langs->trans("Bojanë"),
    25 => $langs->trans("Bukoviq"),
    27 => $langs->trans("Semenishtë"),
    28 => $langs->trans("Çajlanë"),
    29 => $langs->trans("Llaskarcë"),
    31 => $langs->trans("Sfillareeepërme"),
    32 => $langs->trans("Sfillareeposhtme"),
    33 => $langs->trans("Kopanicë"),
  ];
...
function getClientType() {
  ...
  $clientType = [
    1 => $langs->trans("Gurbetqar"),
    2 => $langs->trans("Rastsocial"),
  ];
