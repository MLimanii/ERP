...
class Societe extends CommonObject
{
    ...
    'first_name' =>array('type'=>'varchar(50)', 'label'=>'First name', 'enabled'=>1, 'visible'=>-1, 'position'=>37, 'showoncombobox'=>2),
    'last_name' =>array('type'=>'varchar(50)', 'label'=>'Last name', 'enabled'=>1, 'visible'=>-1, 'position'=>38, 'showoncombobox'=>2),
    'middle_name' =>array('type'=>'varchar(50)', 'label'=>'Middlee name', 'enabled'=>1, 'visible'=>-1, 'position'=>39, 'showoncombobox'=>2),
    ...
    'embg' =>array('type'=>'varchar(14)', 'label'=>'Embg', 'enabled'=>1, 'visible'=>-1, 'position'=>41, 'showoncombobox'=>2),
    'embs' =>array('type'=>'integer(7)', 'label'=>'Embs', 'enabled'=>1, 'visible'=>-1, 'position'=>44, 'showoncombobox'=>2),
    'edb' =>array('type'=>'varchar(14)', 'label'=>'Edb', 'enabled'=>1, 'visible'=>-1, 'position'=>45, 'showoncombobox'=>2),
    'country' =>array('type'=>'varchar(50)', 'label'=>'Country', 'enabled'=>1, 'visible'=>-1, 'position'=>42, 'showoncombobox'=>1),
    'client_type' =>array('type'=>'integer(4)', 'label'=>'ClientType', 'enabled'=>1, 'visible'=>-1, 'position'=>43, 'showoncombobox'=>1),
    'subject_type' =>array('type'=>'integer(4)', 'label'=>'SubjectType', 'enabled'=>1, 'visible'=>-1, 'position'=>46, 'showoncombobox'=>1),
  ...
  /**
   * First names (commercial, trademark or alias names)
   * @var string
   */
  public $first_name;

  /**
   * Last names (commercial, trademark or alias names)
   * @var string
   */
  public $last_name;

  /**
   * Middle names (commercial, trademark or alias names)
   * @var string
   */
  public $middle_name;
  ...
  /**
   * Embg (commercial, trademark or alias names)
   * @var int
   */
  public $embg;
  ...
  /**
   * Country (commercial, trademark or alias names)
   * @var string
   */
  public $custom_contry;

  /**
   * @var string Ovlastenolice code of thirdparty
   */
  public $ovlasteno_lice;

  /**
   * @var int OvlastenoliceEmbg code of thirdparty
   */
  public $ovlasteno_lice_embg;

  /**
   * @var string OvlastenoliceSediste code of thirdparty
   */
  public $ovlasteno_lice_address;
  ...
  /**
	...
	public function create(User $user)
	{
      ...
      $sql .= ", custom_contry";
      $sql .= ", ovlasteno_lice";
      $sql .= ", ovlasteno_lice_embg";
      $sql .= ", ovlasteno_lice_address";
			$sql .= ", entity";
			$sql .= ", datec";
			$sql .= ", fk_user_creat";
			$sql .= ", fk_typent";
      ...
      $sql .= ", subject_type";
			...
			$sql .= ") VALUES ('".$this->db->escape($this->name)."', '".$this->db->escape($this->first_name)."', '".$this->db->escape($this->last_name)."', '".$this->db->escape($this->middle_name)."', '".$this->db->escape($this->name_alias)."', '".getLattin($this->db->escape($this->custom_contry))."', '".$this->db->escape($this->ovlasteno_lice)."', ".((int) $this->entity).", ".((int) $this->ovlasteno_lice_embg).", '".$this->db->escape($this->ovlasteno_lice_address)."', '".$this->db->idate($now)."'";
      ...
      $sql .= ", ".((int) $this->edb);
			$sql .= ", ".((int) $this->status);
      $sql .= ", ".((int) $this->custom_region);
      $sql .= ", ".((int) $this->client_type);
      $sql .= ", ".((int) $this->subject_type);
    ...
    $this->embg = trim($this->embg);
    $this->embs = trim($this->embs);
    $this->edb = trim($this->edb);
    $this->custom_region = trim($this->custom_region);
    $this->client_type = trim($this->client_type);
    $this->subject_type = trim($this->subject_type);
    $this->custom_contry = trim(getLattin($this->custom_contry));
    $this->ovlasteno_lice = trim($this->ovlasteno_lice);
    $this->ovlasteno_lice_embg = trim($this->ovlasteno_lice_embg);
    $this->ovlasteno_lice_address = trim($this->ovlasteno_lice_address);
    ...
    if ($this->client == 1) {
      ...
      $sql .= ",first_name = '".$this->db->escape($this->first_name)."'";
      $sql .= ",last_name = '".$this->db->escape($this->last_name)."'";
      $sql .= ",middle_name = '".$this->db->escape($this->middle_name)."'";
      $sql .= ",embg = '".$this->db->escape($this->embg)."'";
      $sql .= ",embs = '".$this->db->escape($this->embs)."'";
      $sql .= ",edb = '".$this->db->escape($this->edb)."'";
      $sql .= ",custom_region = '".$this->db->escape($this->custom_region)."'";
      $sql .= ",custom_contry = '".getLattin($this->db->escape($this->custom_contry))."'";
      $sql .= ",ovlasteno_lice = '".getLattin($this->db->escape($this->ovlasteno_lice))."'";
      $sql .= ",ovlasteno_lice_embg = '".getLattin($this->db->escape($this->ovlasteno_lice_embg))."'";
      $sql .= ",ovlasteno_lice_address = '".$this->db->escape($this->ovlasteno_lice_address)."'";
      $sql .= ",client_type = '".$this->db->escape($this->client_type)."'";
      $sql .= ",subject_type = '".$this->db->escape($this->subject_type)."'";
	...
	public function fetch($rowid, $ref = '', $ref_ext = '', $barcode = '', $idprof1 = '', $idprof2 = '', $idprof3 = '', $idprof4 = '', $idprof5 = '', $idprof6 = '', $email = '', $ref_alias = '')
	{
		...
		$sql = 'SELECT s.rowid, s.nom as name, s.first_name, s.last_name, s.middle_name, s.embg, s.embs, s.edb, s.custom_region, s.custom_contry, s.ovlasteno_lice, s.ovlasteno_lice_embg, s.ovlasteno_lice_address, s.client_type, s.subject_type, s.name_alias, s.entity, s.ref_ext, s.address, s.datec as date_creation, s.prefix_comm';
    ...
    if ($ref) {
      $sql .= " AND s.first_name = '".$this->db->escape($ref)."'";
    }
		if ($ref_alias) {
        ...
        $this->embs = $obj->embs;
        $this->edb = $obj->edb;
        $this->custom_region = $obj->custom_region;
        $this->custom_contry = $obj->custom_contry;
        $this->ovlasteno_lice = $obj->ovlasteno_lice;
        $this->ovlasteno_lice_embg = $obj->ovlasteno_lice_embg;
        $this->ovlasteno_lice_address = $obj->ovlasteno_lice_address;
        $this->client_type = $obj->client_type;
        $this->subject_type = $obj->subject_type;
    ...
    if (empty($name)) {
      $name = $this->first_name ? $this->first_name : $this->first_name;
    }
    ...
    $typeComArray = getClientType();
    $subjectTypeArray = getSubjectType();

    if ($this->client == 2) {
      $label .= '<br>';
      if (!empty($items['nom'])) {
        $label .= '<br><b>' . $langs->trans('ThirdPartyName') . ':</b> ' . dol_escape_htmltag($items['nom']);
      }
      if (!empty($items['code_client'])) {
        $label .= '<br><b>' . $langs->trans('CustomerCode') . ':</b> ' . dol_escape_htmltag($items['code_client']);
      }
      if (!empty($items['embs'])) {
        $label .= '<br><b>' . $langs->trans('Embs') . ':</b> ' . dol_escape_htmltag($items['embs']);
      }
      if (!empty($items['edb'])) {
        $label .= '<br><b>' . $langs->trans('Edb') . ':</b> ' . dol_escape_htmltag($items['edb']);
      }
      if (!empty($items['address'])) {
        $label .= '<br><b>' . $langs->trans('Sediste') . ':</b> ' . dol_escape_htmltag($items['address']);
      }
      if (!empty($items['custom_region'])) {
        $label .= '<br><b>' . $langs->trans('Region') . ':</b> ' . dol_escape_htmltag($regionArray[$items['custom_region']]);
      }
      if (!empty($items['zip'])) {
        $label .= '<br><b>' . $langs->trans('Zip') . ':</b> ' . dol_escape_htmltag($items['zip']);
      }
      if (!empty($items['town'])) {
        $label .= '<br><b>' . $langs->trans('Town') . ':</b> ' . dol_escape_htmltag($items['town']);
      }
      if (!empty($items['custom_contry'])) {
        $label .= '<br><b>' . $langs->trans('Country') . ':</b> ' . dol_escape_htmltag($items['custom_contry']);
      }
      if (!empty($items['phone'])) {
        $label .= '<br><b>' . $langs->trans('Phone') . ':</b> ' . dol_escape_htmltag($items['phone']);
      }
      if (!empty($items['email'])) {
        $label .= '<br><b>' . $langs->trans('EMail') . ':</b> ' . dol_escape_htmltag($items['email']);
      }
      if (!empty($items['subject_type']) && $items['subject_type'] > 0 ) {
        $label .= '<br><b>' . $langs->trans('Subjecttype') . ':</b> ' . dol_escape_htmltag($subjectTypeArray[$items['subject_type']]);
      }
      if (!empty($items['ovlasteno_lice'])) {
        $label .= '<br><b>' . $langs->trans('Ovlastenolice') . ':</b> ' . dol_escape_htmltag($items['ovlasteno_lice']);
      }
      if (!empty($items['ovlasteno_lice_embg'])) {
        $label .= '<br><b>' . $langs->trans('OvlastenoliceEmbg') . ':</b> ' . dol_escape_htmltag($items['ovlasteno_lice_embg']);
      }
      if (!empty($items['ovlasteno_lice_address'])) {
        $label .= '<br><b>' . $langs->trans('OvlastenoliceSediste') . ':</b> ' . dol_escape_htmltag($items['ovlasteno_lice_address']);
      }
      $label .= '<br /><br />';
    } else {
      $label .= '<br>';
      if (!empty($items['first_name'])) {
        $label .= '<br><b>' . $langs->trans('Firstname') . ':</b> ' . dol_escape_htmltag($items['first_name']);
      }
      if (!empty($items['middle_name'])) {
        $label .= '<br><b>'.$langs->trans('Middlename').':</b> '.dol_escape_htmltag($items['middle_name']);
      }
      if (!empty($items['last_name'])) {
        $label .= '<br><b>' . $langs->trans('LastName') . ':</b> ' . dol_escape_htmltag($items['last_name']);
      }
      if (!empty($items['code_client'])) {
        $label .= '<br><b>' . $langs->trans('CustomerCode') . ':</b> ' . dol_escape_htmltag($items['code_client']);
      }
      if (!empty($items['embg'])) {
        $label .= '<br><b>' . $langs->trans('Embg') . ':</b> ' . dol_escape_htmltag($items['embg']);
      }
      if (!empty($items['address'])) {
        $label .= '<br><b>' . $langs->trans('Address') . ':</b> ' . dol_escape_htmltag($items['address']);
      }
      if (!empty($items['custom_region'])) {
        $label .= '<br><b>' . $langs->trans('Region') . ':</b> ' . dol_escape_htmltag($regionArray[$items['custom_region']]);
      }
      ...
      if (!empty($items['client_type']) && $items['client_type'] > 0 ) {
        $label .= '<br><b>' . $langs->trans('ThirdPartyType') . ':</b> ' . dol_escape_htmltag($typeComArray[$items['client_type']]);
      }
      $label .= '<br /><br />';
    }
...
//		}
	...
	public function getTypeUrl($withpicto = 0, $option = '', $notooltip = 0, $tag = 'a')
	{
		...
		if (empty($option) || preg_match('/prospect/', $option)) {
			if (($this->client == 2 || $this->client == 3) && empty($conf->global->SOCIETE_DISABLE_PROSPECTS)) {
				$s .= '<'.$tag.' class="customer-back tag_pravno" title="'.$langs->trans("Prospect").'" href="'.DOL_URL_ROOT.'/comm/card.php?socid='.$this->id.'">'. $langs->trans("Pravnolice") .'</'.$tag.'>';
			}
		}
		if (empty($option) || preg_match('/customer/', $option)) {
			if (($this->client == 1 || $this->client == 3) && empty($conf->global->SOCIETE_DISABLE_CUSTOMERS)) {
				$s .= '<'.$tag.' class="customer-back tag_fizicko" title="'.$langs->trans("Customer").'" href="'.DOL_URL_ROOT.'/comm/card.php?socid='.$this->id.'">'. $langs->trans("Fizickolice") .'</'.$tag.'>';
			}
		}
  ...
  public function getItems($itemId) {
    ...
    $sql = "SELECT * FROM ".MAIN_DB_PREFIX."societe WHERE rowid = $itemId";
    $resql = $this->db->query($sql);
    if ($resql) {
      $rows = $this->db->fetch_array($itemId);
      if ($rows > 0) {
        return $rows;
      }
    } else {
  ...
  public function if_embg_exists($embgId) {
    $embgId != '';

    $sql = "SELECT * FROM ".MAIN_DB_PREFIX."societe WHERE embg = $embgId";
    $resql = $this->db->query($sql);
    if ($resql) {
      $rows = $this->db->fetch_array($embgId);
      if ($rows > 0) {
        return $rows['embg'];
      }
    } else {
  ...
  public function if_embs_exists($embsId) {
    $embsId != '';

    if (!empty($embsId)) {
      $sql = "SELECT * FROM ".MAIN_DB_PREFIX."societe WHERE embs = $embsId";
      $resql = $this->db->query($sql);
      if ($resql) {
        $rows = $this->db->fetch_array($embsId);
        if ($rows > 0) {
          return $rows['embs'];
        }
      } else {
  ...
  public function if_edb_exists($edbId) {
    $edbId != '';

    if (!empty($embsId)) {
      $sql = "SELECT * FROM " . MAIN_DB_PREFIX . "societe WHERE edb = $edbId";
      $resql = $this->db->query($sql);
      if ($resql) {
        $rows = $this->db->fetch_array($edbId);
        if ($rows > 0) {
          return $rows['edb'];
        }
      }
    ...
    $this->custom_contry = $customContry;
    $this->ovlasteno_lice = $ovlastenoLice;
    $this->ovlasteno_lice_embg = $ovlastenoLiceEmbg;
    $this->ovlasteno_lice_address = $ovlastenoLiceAddress;
